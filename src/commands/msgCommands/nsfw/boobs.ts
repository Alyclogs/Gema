...
export default new Command({
    name: 'boobs',
    description: 'Toca los pechos de alguien en roleplay',
    uso: '',
    aliases: [],
    nsfw: true,
    timeout: 0,
    ...
    async run({ client, message, args, emojis, color }) {
        ...
        if (user) {
            ...
            const media = await fetchNsfwMedia(nsfw.boobs)
            if (!media) return message.reply(`${emojis.error} No pude cargar una imagen, intenta de nuevo`)

            const descriptions = [
                `**${message.member?.displayName}** toca los pechos de **${user.displayName}**`,
                `**${message.member?.displayName}** juega con los pechos de **${user.displayName}**`,
                `**${message.member?.displayName}** aprieta los pechos de **${user.displayName}** sin pedir permiso`
            ]
