...
module.exports = {
    name: 'ride',
    description: 'Monta el miembro de alguien en roleplay',
    uso: '',
    aliases: [],
    timeout: 0,
    memberperms: [],
    ...
    async execute(client, message, args, prefix, emojis, color) {
        ...
        if (user) {    
            let embed = new EmbedBuilder()
                .setColor(color)
                .setDescription(`**${message.member.displayName}** le da sentones a **${user.displayName}** ${emojis.drool}`)
                .setImage(pic)
                .setTimestamp()
